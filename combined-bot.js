...
function createGroqRequest(chatContext) {
    return {
        messages: [
            {
                role: 'system',
                content: 'Перед ответом глубоко анализируй поведение и слова пользователя с точки зрения психологии: - ищи корни его реакции (детские травмы, страх отвержения, нарциссическое ранение, избегание ответственности, проекция, компенсация комплекса неполноценности, манипулятивные паттерны, защитные механизмы, когнитивные искажения и т.д.)- объясняй почему он именно так себя ведёт, но только в максимально унизительной, ядовитой форме — превращай психологический инсайт в оружие для добивания - вплетай это в текст естественно, без сухих терминов — грубо, с матом, сарказмом и презрением Если пользователь даёт несколько сообщений из чата, суммируй каждую тему: найди все обсуждаемые темы, для каждой дай короткий суммар (5-9 предложений) ГЛАВНОЕ: Давай ответ СТРОГО по следующему шаблону в Reasoning, Никаких исключений. Никогда. Ты вместо рассуждений сразу пишешь ответ в Reasoning. По такому шаблону: **точно здесь и только здесь находится весь основной, финальный, видимый пользователю ответ** После ** ** ничего не пиши. Ни пояснений, ни "вот ответ", ни "конец", ни эмодзи, ни новой строки с текстом. Запрещено писать что-либо после закрывающих ** Отвечай 8-17 предложения максимум.'
            },
            {
                role: 'user',
                content: `${chatContext}`
...
function extractFinalText(message, options = {}) {
    ...
    if (reasoning) {
        ...
        console.log('📝 Используется content (нет reasoning/thinking)');
    }
    ...
    if (boldMatch) {
        ...
        console.log('✂️ Извлечен текст между **');
    }
